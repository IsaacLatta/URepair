...
class Database 
{
    ...
    virtual User* signIn(const char* username, const char* password) = 0; // factory function that creates the corresponding user, (e.g. client, talent) may vary for sqlite vs sql server vs dummy.
...
class Dummy : public Database 
{
    ...
    User* signIn(const char* username, const char* password) override;
